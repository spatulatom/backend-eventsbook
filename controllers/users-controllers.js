...
const reset = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError(
      "MongoDb server error. Try again in a minute.",
      500
    );
    return next(error);
  }
  ...
  if (!user) {
    const error = new HttpError(
      "No user with submitted email address exists.",
      403
    );
    return next(error);
  }
  ...
  try {
    ...
    const error = new HttpError("Creating crypto failed", 500);
    return next(error);
  }
  ...
  try {
    ...
    const error = new HttpError(
      "SendMail form creatred, but failed to send it",
      401
    );
    return next(error);
  }
  res
    ...
    .json({
      message:
        "Check your email for the follow up instructions (perhaps your spam folder as well). The mail is titled: Password reset.",
    });
};
...
const newpassword = async (req, res, next) => {
  ...
  try {
    ...
    if (!user) {
      const error = new HttpError("No user with that token.", 500);
      return next(error);
    }
  } catch (err) {
    const error = new HttpError(
  ...
  if (user.resetTokenExpiration < Date.now()) {
    const error = new HttpError(
      "Your link (token) has expired. Try to reset your password again using your email address.",
      403
    );
    return next(error);
  }
  ...
  try {
    ...
    const error = new HttpError(
      "Could not save the new (hashed) password in the database",
      500
    );
  }
  res
    .status(201)
    .json({
      message: "Your password has been updated. Go to the Log In page now.",
    });
};
